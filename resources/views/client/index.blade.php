    ...
    <x-dashboard.sidebar>
        ...
        <div
            ...
            <div class="flex flex-row gap-2 sm:gap-3 items-start sm:items-center text-sm">
                <form method="GET" action="{{ route('client.index') }}" class="flex gap-2 items-center"
                ...
                @can('createclient')
                    <a href="{{ route('client.create') }}" onclick="showFullScreenLoader();"
                ...
                <div class="overflow-hidden shadow rounded-lg" id="clientList" data-list>
                    <table
                        ...
                        <tbody id="clientBody"
                            ...
                            data-changes-url="{{ route('client.sync.changes', request()->only('search')) }}"
                            data-rows-url="{{ route('client.rows') }}">
                            @forelse ($clients as $index => $client)
                                <tr data-id="{{ $client->id }}">
                                    <td class="px-4 py-3 text-left no">
                                        {{ $loop->iteration + ($clients instanceof \Illuminate\Pagination\LengthAwarePaginator ? ($clients->currentPage() - 1) * $clients->perPage() : 0) }}
                                    </td>
        ...
        <x-per-page-selector :route="'client.index'" :perPage="$perPage" :search="$search" :items="$clients" />
