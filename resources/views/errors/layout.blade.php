...
<head>
    ...
    <title>@yield('code') &mdash; Portal Immanuel Production</title>
    <script>
        (() => {
...
<body class="flex min-h-full items-center justify-center bg-sky-50 p-5 font-sans text-slate-900 dark:bg-[#080b12] dark:text-white">
    <main class="w-full max-w-xl overflow-hidden rounded-[2rem] border border-sky-100 bg-white p-6 text-center shadow-2xl shadow-sky-900/10 dark:border-white/10 dark:bg-[#11151e] dark:shadow-black/30 sm:p-10">
        <div class="mx-auto flex h-16 w-fit items-center gap-3 rounded-2xl bg-[#0b0c0f] px-4">
            <img src="{{ asset('assets/brand/immanuel-production-legacy-logo.png') }}" alt="Immanuel Production" class="h-12 w-12 object-contain">
        ...
        <p class="mt-8 text-xs font-extrabold uppercase tracking-[.28em] text-sky-600 dark:text-red-400">Kode @yield('code')</p>
        <h1 class="mt-3 text-2xl font-extrabold tracking-tight sm:text-3xl">@yield('title')</h1>
        <p class="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">@yield('message')</p>
        <div class="mt-7 flex flex-col justify-center gap-2 sm:flex-row">
            ...
            <a href="{{ route('dashboard') }}" class="ip-btn-primary">Ke dashboard</a>
        </div>
    </main>
</body>
</html>
