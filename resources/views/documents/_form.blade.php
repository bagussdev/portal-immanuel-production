...
<form method="POST" action="{{ $action }}" id="documentForm" class="space-y-6">
    ...
    <x-responsive-disclosure
        kicker="Informasi utama"
        title="Client & acara"
        description="Nama client baru akan tersimpan otomatis."
        :mobile-open="true"
    >
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Nama acara
                <input name="event_name" value="{{ old('event_name', $document->event_name) }}" class="ip-input mt-1" placeholder="Contoh: Wedding Andi & Sinta">
            </label>
        </div>
    </x-responsive-disclosure>
    ...
    <x-responsive-disclosure
        kicker="Operasional"
        title="{{ $isInvoice ? 'Jadwal & tipe event' : 'Jadwal pekerjaan' }}"
        description="Buka saat perlu mengatur lokasi, tanggal, dan alur tim lapangan."
    ...
    >
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Lokasi
                <input name="location_event" value="{{ old('location_event', $document->location_event) }}" class="ip-input mt-1" placeholder="Lokasi pekerjaan">
            </label>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Tanggal acara
                <input type="date" name="event_date" value="{{ old('event_date', optional($document->event_date)->format('Y-m-d')) }}" class="ip-input mt-1">
            </label>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Jadwal loading
                <input type="datetime-local" name="loading_date" value="{{ old('loading_date', optional($document->loading_date)->format('Y-m-d\TH:i')) }}" class="ip-input mt-1">
            </label>
            <label id="bongkaranField" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Jadwal bongkar
                <input type="datetime-local" name="bongkaran_date" value="{{ old('bongkaran_date', optional($document->bongkaran_date)->format('Y-m-d\TH:i')) }}" class="ip-input mt-1">
            </label>
            @if($isInvoice)
                    ...
                    <div class="grid gap-3 sm:grid-cols-3">
                        @foreach ([
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="work_flow" value="{{ $value }}" class="peer sr-only" @checked($workFlow === $value)>
                                <span class="block h-full rounded-xl border border-slate-200 bg-white p-3 transition peer-checked:border-sky-500 peer-checked:bg-sky-50 peer-checked:ring-2 peer-checked:ring-sky-500/15 dark:border-white/10 dark:bg-white/[.04] dark:peer-checked:border-sky-400 dark:peer-checked:bg-sky-500/10">
                                    <span class="block text-sm font-extrabold text-slate-900 dark:text-white">{{ $label }}</span>
    ...
    <x-responsive-disclosure
        ...
        description="Opsional. Informasi rekening terpilih akan muncul di detail dan PDF."
    ...
    <x-responsive-disclosure kicker="Rincian" title="Item pekerjaan" description="Geser tabel ke kanan atau kiri untuk mengisi seluruh kolom." :mobile-open="true">
    ...
    <section class="grid gap-6 lg:grid-cols-[1fr,420px]">
        <x-responsive-disclosure kicker="Keterangan" title="Catatan tambahan" description="Buka hanya bila ada informasi untuk client atau tim lapangan." :mobile-open="$errors->hasAny(['notes', 'description', 'operational_notes', 'change_reason'])">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Catatan
                <textarea name="{{ $isInvoice ? 'notes' : 'description' }}" rows="5" class="ip-input mt-1" placeholder="Catatan untuk client atau tim internal">{{ old($isInvoice ? 'notes' : 'description', $isInvoice ? $document->notes : $document->description) }}</textarea>
            </label>
            @if($isInvoice)
                <label class="mt-4 block text-sm font-medium text-slate-700 dark:text-slate-200">Catatan khusus tim lapangan
                    <textarea name="operational_notes" rows="4" class="ip-input mt-1" placeholder="Instruksi teknis tanpa harga, pembayaran, atau informasi keuangan">{{ old('operational_notes', $document->operational_notes) }}</textarea>
            ...
            @if ($isInvoice && $document->exists && $document->status !== 'draft')
                <label class="mt-4 block text-sm font-medium text-slate-700 dark:text-slate-200">Alasan perubahan
                    <input name="change_reason" value="{{ old('change_reason') }}" class="ip-input mt-1" placeholder="Opsional, tetapi disarankan untuk audit">
                </label>
            @endif
        </x-responsive-disclosure>

        <x-responsive-disclosure kicker="Perhitungan" title="Ringkasan nilai" description="Diskon, pajak, dan total invoice." :mobile-open="true">
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-[minmax(0,1fr)_72px_minmax(0,1.15fr)] items-end gap-2">
                    <label class="min-w-0 text-xs font-semibold text-slate-500 dark:text-slate-400">Diskon
                        <select name="discount_mode" id="discountMode" class="ip-input mt-1 min-w-0 !py-2 text-xs"><option value="percent" @selected($discountMode === 'percent')>Persen</option><option value="amount" @selected($discountMode === 'amount')>Nominal</option></select>
                    </label>
                    <input type="number" step="0.01" min="0" max="100" name="discount_percent" id="discountPercent" value="{{ old('discount_percent', $document->discount_percent) }}" class="ip-input min-w-0 !py-2 text-right text-sm disabled:bg-slate-100 disabled:text-slate-400 dark:disabled:bg-white/[.04]" placeholder="%">
                    <input name="discount_value" id="discountValue" value="{{ old('discount_value', $discountAmount ?: '') }}" class="ip-input money min-w-0 !py-2 text-right text-sm disabled:bg-slate-100 disabled:text-slate-400 dark:disabled:bg-white/[.04]" placeholder="Rp">
                </div>
                <div class="grid grid-cols-[minmax(0,1fr)_72px_minmax(0,1.15fr)] items-end gap-2">
                    <label class="min-w-0 text-xs font-semibold text-slate-500 dark:text-slate-400">Potongan pajak
                        <select name="tax_mode" id="taxMode" class="ip-input mt-1 min-w-0 !py-2 text-xs"><option value="percent" @selected($taxMode === 'percent')>Persen</option><option value="amount" @selected($taxMode === 'amount')>Nominal</option></select>
                    </label>
                    <input type="number" step="0.01" min="0" max="100" name="tax_percent" id="taxPercent" value="{{ old('tax_percent', $document->tax_percent) }}" class="ip-input min-w-0 !py-2 text-right text-sm disabled:bg-slate-100 disabled:text-slate-400 dark:disabled:bg-white/[.04]" placeholder="%">
                    <input name="tax_value" id="taxValue" value="{{ old('tax_value', $document->tax_value ?: '') }}" class="ip-input money min-w-0 !py-2 text-right text-sm disabled:bg-slate-100 disabled:text-slate-400 dark:disabled:bg-white/[.04]" placeholder="Rp">
                </div>
