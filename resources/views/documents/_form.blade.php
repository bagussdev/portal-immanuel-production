                ...
                <article class="rounded-2xl border border-sky-100 bg-sky-50/35 p-4 dark:border-white/10 dark:bg-white/[.025]">
                    ...
                    <div class="mt-5 space-y-3">
                        <template x-for="(item, itemIndex) in location.items" :key="item.key">
                            <div class="rounded-2xl border border-sky-100 bg-white p-4 shadow-sm transition focus-within:border-sky-400 focus-within:shadow-md dark:border-white/10 dark:bg-[#10141d]">
                                <div class="flex items-start gap-3"><label class="min-w-0 flex-1 text-xs font-extrabold text-slate-500">Nama item<input :name="`locations[${locationIndex}][items][${itemIndex}][item_name]`" x-model="item.item_name" required class="ip-input mt-1 text-base" placeholder="Nama pekerjaan atau barang"></label><button type="button" @click="removeItem(locationIndex,itemIndex)" class="mt-6 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-50 text-xl font-bold text-red-600" aria-label="Hapus item">&times;</button></div>
                                <div class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-[90px,110px,150px,1fr]">
                                    <label class="text-[10px] font-extrabold text-slate-400">Qty<input type="number" min="0.01" step="0.01" :name="`locations[${locationIndex}][items][${itemIndex}][qty]`" x-model.number="item.qty" class="ip-input mt-1"></label>
                                    <label class="text-[10px] font-extrabold text-slate-400">Panjang<input type="number" min="0" step="0.01" :name="`locations[${locationIndex}][items][${itemIndex}][length]`" x-model="item.length" class="ip-input mt-1" placeholder="Kosong"></label>
                                    <label class="text-[10px] font-extrabold text-slate-400">Mode<select :name="`locations[${locationIndex}][items][${itemIndex}][pricing_mode]`" x-model="item.pricing_mode" class="ip-input mt-1"><option value="unit">Harga satuan</option><option value="total">Total langsung</option></select></label>
                                    <label class="text-[10px] font-extrabold text-slate-400"><span x-text="item.pricing_mode === 'total' ? 'Total final' : 'Harga satuan'"></span><input :name="`locations[${locationIndex}][items][${itemIndex}][${item.pricing_mode === 'total' ? 'line_total' : 'unit_price'}]`" :value="item.pricing_mode === 'total' ? item.line_total : item.unit_price" @input="setMoney(item, item.pricing_mode === 'total' ? 'line_total' : 'unit_price', $event.target)" inputmode="numeric" class="ip-input mt-1 text-right" placeholder="Rp"></label>
                                </div>
                                <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
                                    <label x-show="itemIndex > 0" class="inline-flex cursor-pointer items-center gap-2 text-xs font-bold text-slate-500"><input type="checkbox" value="1" :name="`locations[${locationIndex}][items][${itemIndex}][merge_price]`" x-model="item.merge_price" class="rounded border-sky-200 text-sky-600"> Gabung harga dengan item sebelumnya</label>
                                    <strong class="ml-auto text-sm text-sky-700 dark:text-red-400" x-text="item.merge_price ? 'Digabung' : rupiah(lineTotal(item))"></strong>
                                </div>
                            </div>
                        </template>
                        <button type="button" @click="addItem(locationIndex)" class="ip-btn-secondary w-full sm:w-auto">+ Tambah item</button>
                    </div>
                </article>
            </template>
