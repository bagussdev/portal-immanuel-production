@php
    ...
    $imageVersion = $editing ? $user->updated_at?->getTimestamp() : null;
    $profileTransform = $imageTransforms['profile'] ?? ['x' => 50, 'y' => 50, 'zoom' => 1];
    $ktpTransform = $imageTransforms['ktp'] ?? ['x' => 50, 'y' => 50, 'zoom' => 1];
    $profileUrl = $editing && $user->profile_photo_path ? route('users.photo', ['user' => $user, 'kind' => 'profile', 'source' => 'original', 'v' => $imageVersion]) : null;
    $ktpUrl = $editing && $user->ktp_photo_path ? route('users.photo', ['user' => $user, 'kind' => 'ktp', 'source' => 'original', 'v' => $imageVersion]) : null;
@endphp
    ...
    <div class="grid items-start gap-5 xl:grid-cols-[380px,minmax(0,1fr)]">
        <section class="ip-card p-5 sm:p-6">
            <button type="button" @click="identityOpen = !identityOpen"
                ...
                :aria-expanded="identityOpen.toString()" aria-controls="user-identity-panel">
                <span>
            ...
            <div id="user-identity-panel" x-cloak x-show="identityOpen" x-transition.opacity class="mt-4 space-y-6">
                <div>
        ...
        <section class="ip-card self-start p-5 sm:p-6">
            ...
            <div class="mt-6 flex flex-wrap justify-end gap-3 border-t border-sky-100 pt-5 dark:border-white/10">
                <a href="{{ route('users.index') }}" class="ip-btn-secondary">Batal</a>
                <button class="ip-btn-primary">Simpan user</button>
            </div>
        </section>
    </div>
</form>
