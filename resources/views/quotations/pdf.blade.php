    ...
    <div class="content">
                    ...
                    <td style="width:50%; vertical-align:top;">
                        <table>
                            ...
                            <tr>
                                ...
                                <td>:
                                    {{ $quotation->event_date ? \Carbon\Carbon::parse($quotation->event_date)->format('d M Y') : '-' }}
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>: {{ $quotation->location_event ?? '-' }}</td>
                            </tr>
                        </table>
                    </td>
                    <td style="width:50%; vertical-align:top;">
                        <table>
                            ...
                            <tr>
                                ...
                                <td>:
                                    {{ $quotation->quotation_date ? \Carbon\Carbon::parse($quotation->quotation_date)->format('d M Y') : \Carbon\Carbon::now()->format('d M Y') }}
                                </td>
                            </tr>
                        </table>
                    </td>
        ...
        @if ($quotation->bankDetail)
            <div class="bank-detail">
                ...
                <table>
                    @foreach (['Email' => $quotation->bankDetail->email, 'Bank' => $quotation->bankDetail->bank_name, 'Atas Nama' => $quotation->bankDetail->account_name, 'No Rek' => $quotation->bankDetail->account_number, 'NPWP' => $quotation->bankDetail->npwp, 'No HP' => $quotation->bankDetail->phone] as $label => $value)
        ...
        <div class="summary">
            ...
            <table>
                <tr>
                    <td>Subtotal</td>
                    <td>:</td>
                    <td>{{ (int) $quotation->subtotal > 0 ? 'Rp ' . number_format($quotation->subtotal, 0, ',', '.') : '' }}</td>
                </tr>
                @if ((int) $quotation->discount > 0)<tr>
                    <td>Discount ({{ number_format($quotation->discount_percent, 0, ',', '.') }}%)</td>
                    <td>:</td>
                    <td>- Rp {{ number_format($quotation->discount, 0, ',', '.') }}</td>
                </tr>@endif
                @if ((int) $quotation->tax_value > 0)<tr>
                    <td>Potongan Pajak{{ $quotation->tax_percent !== null ? ' (' . number_format($quotation->tax_percent, 2, ',', '.') . '%)' : '' }}</td>
                    <td>:</td>
                    <td>- Rp {{ number_format($quotation->tax_value, 0, ',', '.') }}</td>
                </tr>@endif
                <tr class="emph">
                    <td><strong>Total Penawaran</strong></td>
                    <td>:</td>
                    <td><strong>{{ (int) $quotation->grand_total > 0 ? 'Rp ' . number_format($quotation->grand_total, 0, ',', '.') : '' }}</strong></td>
                </tr>
            </table>
        </div>
