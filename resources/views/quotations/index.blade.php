...
<div class="ip-page">
    <header class="ip-page-header"><div><p class="ip-kicker">Penjualan</p><h1 class="ip-title">Quotation</h1><p class="ip-subtitle">Penawaran yang disetujui otomatis menjadi invoice draft dan tetap dapat disesuaikan.</p></div>@can('createquotation')<a href="{{ route('quotations.create') }}" class="ip-btn-primary">+ Buat quotation</a>@endcan</header>
    <form class="ip-card flex flex-col gap-3 p-4 sm:flex-row"><input name="search" value="{{ $search }}" placeholder="Cari nomor, client, atau acara" class="ip-input flex-1"><select name="status" class="ip-input sm:w-52"><option value="">Semua status</option>@foreach(['draft'=>'Draft','sent'=>'Dikirim','approved'=>'Disetujui','cancelled'=>'Dibatalkan'] as $value=>$label)<option value="{{ $value }}" @selected($status===$value)>{{ $label }}</option>@endforeach</select><button class="ip-btn-dark">Terapkan filter</button></form>
