    ...
    <section class="grid w-full overflow-hidden rounded-[2rem] border border-sky-100 bg-white shadow-[0_32px_90px_-35px_rgba(15,23,42,.35)] lg:grid-cols-[1.08fr_.92fr]">
        <div class="relative hidden min-h-[680px] overflow-hidden bg-gradient-to-br from-slate-950 via-sky-950 to-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
            ...
            <div class="relative">
                <div class="inline-flex items-center gap-4 rounded-2xl border border-white/10 bg-black/50 px-5 py-4 shadow-2xl backdrop-blur">
                    <img src="{{ asset('assets/brand/immanuel-production-white-logo.png') }}" alt="Immanuel Production" class="h-20 w-32 object-contain">
            ...
            <div class="relative max-w-xl">
                ...
                <div class="mt-8 grid grid-cols-3 gap-3">
                    @foreach ([['Dokumen','Rapi'],['Jadwal','Terpantau'],['Akses','Terkontrol']] as [$label,$value])
                        <div class="rounded-2xl border border-white/10 bg-white/[.06] p-4 backdrop-blur"><p class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">{{ $label }}</p><p class="mt-1 text-sm font-extrabold text-white">{{ $value }}</p></div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="flex min-h-[620px] items-center bg-white px-6 py-10 sm:px-12 lg:px-16" x-data="{ showPassword: false }">
            <div class="mx-auto w-full max-w-md">
                <div class="mb-9 inline-flex items-center gap-3 rounded-2xl bg-slate-950 px-4 py-3 shadow-lg lg:hidden"><img src="{{ asset('assets/brand/immanuel-production-white-logo.png') }}" alt="Immanuel Production" class="h-14 w-20 object-contain"><span class="text-left"><strong class="block text-xs tracking-wide text-white">PORTAL IMMANUEL</strong><span class="text-[9px] font-bold tracking-[.2em] text-slate-400">PRODUCTION</span></span></div>
                ...
                <x-auth-session-status class="mt-6 ip-alert-success" :status="session('status')" />
                ...
                @if ($errors->any())
                    <div class="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-900 shadow-sm" role="alert">
                        <div class="flex items-start gap-3">
                            ...
                            <div><p class="font-extrabold">Login belum berhasil</p><p class="mt-1 text-sm font-medium leading-5 text-red-700">{{ $errors->first() }}</p></div>
                        </div>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="mt-7 space-y-5" onsubmit="showFullScreenLoader();">
                    ...
                    <div>
                        <div class="mb-1.5 flex items-center justify-between"><label for="password" class="ip-label !mb-0">Password</label>@if (Route::has('password.request'))<a class="text-xs font-extrabold text-sky-700 hover:text-sky-900" href="{{ route('password.request') }}">Lupa password?</a>@endif</div>
                        <div class="relative">
