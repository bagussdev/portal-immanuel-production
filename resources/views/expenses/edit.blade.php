    ...
    <x-dashboard.sidebar>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <form action="{{ route('expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data"
                ...
                {{-- Kunci periode target (opsional, untuk konsistensi UI) --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="date" name="expense_date"
                            value="{{ old('expense_date', $defaultDate ?? optional($expense->expense_date)->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $expense->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="qty" min="1"
                            value="{{ old('qty', (int) $expense->qty) }}" required
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm" />
                        @error('qty')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="total"
                            value="{{ old('total', number_format((int) $expense->total, 0, ',', '.')) }}" required
                ...
                {{-- Attachment (samakan dengan create) --}}
                <div class="mt-4">
                    ...
                    @if ($expense->attachment)
                        <div class="text-xs mb-1">
                            File saat ini:
                            <a href="{{ route('expenses.attachment', $expense) }}" target="_blank"
                ...
                <div class="mt-4">
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm"
                        placeholder="Keterangan tambahan...">{{ old('notes', $expense->notes) }}</textarea>
                    @error('notes')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <x-action-button text="Update" color="green" />
                </div>
            </form>
        </div>
